...
aggregated_insurance = pd.DataFrame(table1, columns=("States","Years","Quarter","Insurance_Type","Transaction_Count","Transaction_Amount"))
...
def Aggregated_insurance_Y(df,year):
    ...
    with col1:
        ...
        fig_amount= px.area(aiyg, x="States", y= "Transaction_Amount",title= f"{year} TRANSACTION AMOUNT",
                           width=600, height= 650, color_discrete_sequence=px.colors.sequential.Aggrnyl)
        st.plotly_chart(fig_amount)
    with col2:

        fig_count= px.area(aiyg, x="States", y= "Transaction_Count",title= f"{year} TRANSACTION COUNT",
    ...
    with col1:
        ...
        india_map_1= px.choropleth(aiyg, geojson= data1, locations= "States", featureidkey= "properties.ST_NM",
    ...
    with col2:
        ...
        india_map_2= px.choropleth(aiyg, geojson= data1, locations= "States", featureidkey= "properties.ST_NM",
...
def Aggregated_insurance_Y_Q(df,quarter):
    ...
    with col1:
        fig_q_amount= px.histogram(aiyqg, x= "States", y= "Transaction_Amount", 
                            title= f"{aiyq['Years'].min()} AND {quarter} TRANSACTION AMOUNT",width= 600, height=650,
                            color_discrete_sequence=px.colors.sequential.Burg_r)
        st.plotly_chart(fig_q_amount)

    with col2:
        fig_q_count= px.histogram(aiyqg, x= "States", y= "Transaction_Count", 
    ...
    with col1:
        ...
        india_map_1= px.choropleth(aiyqg, geojson= data1, locations= "States", featureidkey= "properties.ST_NM",
                                 color= "Transaction_Amount", color_continuous_scale= "Sunsetdark",
    ...
    with col2:
        ...
        india_map_2= px.choropleth(aiyqg, geojson= data1, locations= "States", featureidkey= "properties.ST_NM",
                                 color= "Transaction_Count", color_continuous_scale= "Sunsetdark",
...
def ques2():
    ...
    fig_lts= px.bar(lt2, x= "States", y= "Transaction_Amount",
...
def ques7():
    ...
    fig_stc= px.bar(stc2, x= "States", y= "Transaction_Count", 
                    title= "State Wth Lowest Transaction Count",
...
def ques9():
    ...
    fig_lts= px.bar(ht2, x= "States", y= "Transaction_Amount",
...
def ques10():
    ...
    fig_dt= px.bar(dt2, x= "Districts", y= "Transaction_Amount", 
...
with st.sidebar:
    st.sidebar.image(add_logo(logo_path="C:/Users/Balaji/Downloads/Youtubeproject/PhonePe-Logo.wine.png", width=250, height=150)) 
...
if select=="HOME":
    ...
    with col1:
        st.video("C:\\Users\\Balaji\\Downloads\\Youtubeproject\\You can quickly pay with no PIN with UPI lite on PhonePe.mp4")
       
    with col2:
        st.subheader(":rainbow[INDIA'S BEST TRANSACTION APP]")
        st.markdown("PhonePe  is an Indian digital payments and financial technology company")
        st.write('****Features****:sunglasses:',)
    ...
    with col3:
        st.video("C:\\Users\\Balaji\\Downloads\\Youtubeproject\\Phone Pe Ad.mp4")
    ...
    with tab1:
        ...
        if method1 == "Aggregated Insurance Analysis":
            col1,col2 = st.columns(2)
            with col1:
                years = st.slider("**Select the year**",aggregated_insurance["Years"].min(),
                                  aggregated_insurance["Years"].max(),aggregated_insurance["Years"].min())
                df_aggregated_insurance_year = Aggregated_insurance_Y(aggregated_insurance,years)
            col1,col2= st.columns(2)
            with col1:
                quarters= st.slider("**Select the Quarter**", df_aggregated_insurance_year["Quarter"].min(),
                                     df_aggregated_insurance_year["Quarter"].max(),
                                     df_aggregated_insurance_year["Quarter"].min())

            Aggregated_insurance_Y_Q(df_aggregated_insurance_year, quarters)
            ...
            col1,col2= st.columns((2))
            with col1:
                years_aggregated_transaction= st.slider("**Select the Year**", aggregated_transaction["Years"].min(),
                                                         aggregated_transaction["Years"].max(),
                                                         aggregated_transaction["Years"].min())

            df_agg_tran_Y= Aggregated_insurance_Y(aggregated_transaction,years_aggregated_transaction)
            
            col1,col2= st.columns(2)
            with col1:
                quarters_aggregated_trans= st.slider("**Select the Quarter**", df_agg_tran_Y["Quarter"].min(), 
                                       df_agg_tran_Y["Quarter"].max(),
                                       df_agg_tran_Y["Quarter"].min())

            df_agg_tran_Y_Q= Aggregated_insurance_Y_Q(df_agg_tran_Y, quarters_aggregated_trans)

            #Select the State for Analyse the Transaction type

            state_Y_Q= st.selectbox("**Select the State**",df_agg_tran_Y_Q["States"].unique())

            Aggregated_Transaction_type(df_agg_tran_Y_Q,state_Y_Q)

        elif method1 == "Aggregated User Analysis":
            year_ua= st.selectbox("Select the Year_UA",aggregated_user["Years"].unique())
            agg_user_Y= Aggregated_user_plot_1(aggregated_user,year_ua)
        
            quarter_ua= st.selectbox("Select the Quarter_UA",agg_user_Y["Quarter"].unique())
            agg_user_Y_Q= Aggregated_user_plot_2(agg_user_Y,quarter_ua)
        
            state_ua= st.selectbox("**Select the State_UA**",agg_user_Y["States"].unique())
            Aggregated_user_plot_3(agg_user_Y_Q,state_ua)
    ...
    with tab2:
        ...
        if method2 == "Map Insurance Analysis":
            col1,col2= st.columns(2)
            with col1:
                years_m1= st.slider("**Select the Year**", map_insurance["Years"].min(), 
                                    map_insurance["Years"].max(),map_insurance["Years"].min())

            df_map_insur_Y= Aggregated_insurance_Y(map_insurance,years_m1)

            col1,col2= st.columns(2)
            with col1:
                state_m1= st.selectbox("Select the State", df_map_insur_Y["States"].unique())

            map_insurance_plot_1(df_map_insur_Y,state_m1)

            col1,col2= st.columns(2)
            with col1:
                quarters_m1= st.slider("**Select the Quarter**", df_map_insur_Y["Quarter"].min(), 
                                       df_map_insur_Y["Quarter"].max(),df_map_insur_Y["Quarter"].min())

            df_map_insur_Y_Q= Aggregated_insurance_Y_Q(df_map_insur_Y, quarters_m1)

            col1,col2= st.columns(2)
            with col1:
                state_m2= st.selectbox("Select the State", df_map_insur_Y_Q["States"].unique())            
            
            map_insurance_plot_2(df_map_insur_Y_Q, state_m2)

        elif method2 == "Map Transaction Analysis":
            col1,col2= st.columns(2)
            with col1:
                years_m2= st.slider("**Select the Year**", map_transaction["Years"].min(), 
                                    map_transaction["Years"].max(), map_transaction["Years"].min())

            df_map_tran_Y= Aggregated_insurance_Y(map_transaction, years_m2)

            col1,col2= st.columns(2)
            with col1:
                state_m3= st.selectbox("Select the State", df_map_tran_Y["States"].unique())

            map_insurance_plot_1(df_map_tran_Y,state_m3)

            col1,col2= st.columns(2)
            with col1:
                quarters_m2= st.slider("**Select the Quarter**", df_map_tran_Y["Quarter"].min(), 
                                       df_map_tran_Y["Quarter"].max(),df_map_tran_Y["Quarter"].min())

            df_map_tran_Y_Q= Aggregated_insurance_Y_Q(df_map_tran_Y, quarters_m2)

            col1,col2= st.columns(2)
            with col1:
                state_m4= st.selectbox("Select the State", df_map_tran_Y_Q["States"].unique())            
            
            map_insurance_plot_2(df_map_tran_Y_Q, state_m4)
    ...
    with tab3:
        ...
        if method3 == "Top Insurance Analysis":
            col1,col2= st.columns(2)
            with col1:
                years_t1= st.slider("**Select the Year**", top_insurance["Years"].min(), 
                                    top_insurance["Years"].max(),top_insurance["Years"].min())
 
            df_top_insur_Y= Aggregated_insurance_Y(top_insurance,years_t1)

            
            col1,col2= st.columns(2)
            with col1:
                quarters_t1= st.slider("**Select the Quarter**", df_top_insur_Y["Quarter"].min(),
                                        df_top_insur_Y["Quarter"].max(),df_top_insur_Y["Quarter"].min())

            df_top_insur_Y_Q= Aggregated_insurance_Y_Q(df_top_insur_Y, quarters_t1)
        elif method3 == "Top Transaction Analysis":
            col1,col2= st.columns(2)
            with col1:
                years_t2= st.slider("**Select the Year**", top_transaction["Years"].min(), 
                                    top_transaction["Years"].max(),top_transaction["Years"].min())
 
            df_top_tran_Y= Aggregated_insurance_Y(top_transaction,years_t2)
            
            col1,col2= st.columns(2)
            with col1:
                quarters_t2= st.slider("**Select the Quarter**", df_top_tran_Y["Quarter"].min(), 
                                       df_top_tran_Y["Quarter"].max(),df_top_tran_Y["Quarter"].min())

            df_top_tran_Y_Q= Aggregated_insurance_Y_Q(df_top_tran_Y, quarters_t2)
...
if select=="TOP CHARTS":
    ...
    question= st.selectbox("**Select the Question**",('Top Brands Of Mobiles Used','States With Lowest Trasaction Amount',
    ...
    if question=="Top Brands Of Mobiles Used":
        ques1()

    elif question=="States With Lowest Trasaction Amount":
        ques2()

    elif question=="Districts With Highest Transaction Amount":
        ques3()

    elif question=="Top 10 Districts With Lowest Transaction Amount":
        ques4()

    elif question=="Top 10 States With AppOpens":
        ques5()

    elif question=="Least 10 States With AppOpens":
        ques6()

    elif question=="States With Lowest Trasaction Count":
        ques7()

    elif question=="States With Highest Trasaction Count":
        ques8()

    elif question=="States With Highest Trasaction Amount":
        ques9()

    elif question=="Top 50 Districts With Lowest Transaction Amount":
        ques10()
